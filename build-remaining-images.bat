@echo off
REM Script to build remaining Docker images (after JavaScript and Python)
REM Run this script from the codingPlatformNode directory

echo Building remaining Docker images for code execution...
echo.
echo Already built: javascript-compiler, python-compiler
echo.
...
if %errorlevel% neq 0 (
    echo Warning: Failed to build java-compiler
    echo You can retry later or skip Java for now
    echo.
)
...
echo.
echo Remaining Docker images built successfully!
echo.
echo Built images:
